  ...
  bool is_signed)
{
  ...
  {
    result=bv_utils.extract(result, 0, dest_width);
  }
...
void float_utilst::round_fraction(unbiased_floatt &result)
{
  ...
  else // fraction gets smaller -- rounding
  {
    ...
    assert(extra_bits>=1);
    literalt rounding_bit=result.fraction[extra_bits-1];
    ...
    literalt rounding_least=result.fraction[0];
    literalt increment;
